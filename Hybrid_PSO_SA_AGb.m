function [GBFO,GBest,FO]=Hybrid_PSO_SA_AGb(DataProject,RRHH,TA,CP,CAType,SS,CI,SWP,FE) 
...
% Descripci??n: Algoritmo hibrido PSO para la soluci??n del problema SDPSP
% Entradas
...
[Swarm,SwarmPB,GBest,GB,NP,CA,T] = SwarmCreatorNew(DataProject,RRHH,TA,SS,CP,CAType,FE); % Crea el enjambre O(4n???????)

for i=1:CI % Para cada una de las iteraciones O(n)
    
    Alpha=((0.5-SWP(4))/CI^2)*(CI-i)^2 + SWP(4); % 0.15-0.5
     ...
     for j=1:SS % Para cada una de las part??culas O(n)      
         ...
         m=(Swarm(j).FO-GBest.FO)/(GBest.FO+Swarm(j).FO);
          ...
          [Swarm(j)]=SSGSSwarm(Swarm(j),GBest,SwarmPB(j),DataProject,RRHH,TA,NP,CA,CP,X,R1,R2,Alpha,T);
...
function [Swarm,SwarmPB,GBest,GB,NP,CA,T] = SwarmCreatorNew(DataProject,RRHH,TA,SS,CP,CAType,FE)
...
[WFO,W]=max([Swarm(1:SS).FO]); % GBFO Valor de la funci??n objetivo de la mejor part??cula; PGB particula con la mejor evalucion global
...
Worst=Swarm(W);
T=-max(mean(Worst.TF-GBest.TF))/log(0.9);
end
...
function[CurrParticle] = SSGSSwarm(CurrParticle,GBest,PBest,DataProject,RRHH,TA,NP,CA,CP,X,R1,R2,Alpha,To)
...
TimeNew=cat(3,zeros(NP,CA),zeros(NP,CA),CurrParticle.RA);

TimeOld=cat(3,zeros(NP,CA),zeros(NP,CA),CurrParticle.RA);
...
for k=nonzeros(ind)' % Para cada uno de los Proyectos
    ...
    for i=OrdenP % Para proyectos Iniciado segun OrdenP
     ...
     [Schedule,TimeNew,APP,ARP] = SchedulingFeasibleSwarmA(Schedule,PBest,GBest,DataProject,RRHH,TimeOld,TimeNew,i,ARP(i,:),ARP,APP,X,R1,R2,Alpha,To);
...
function [Schedule,TimeNew,APP,ARP] = SchedulingFeasibleSwarmA(Schedule,PBest,GBest,DataProject,RRHH,TimeOld,TimeNew,i,j,ARP,APP,X,R1,R2,Alpha,T)
%  SchedulingFeasible Summary of this function 
%  Autor: Lic Bolivar E. Medrano Broche. CEGEL & Dpto. Ciencias B??sicas.
%  Facultad III. Universidad de las Ciencias Inform??ticas
      ...
      if CAP==length(AA) && APP(i,Act(k))==0 % si est?? lista y nunca ha sido planificada
       ...
       %fprintf(1,'Alpha %2.4g\n',Alpha)
        ...
        %   disp('Gbest')
           ...
           TimeNew(i,Act(k),3)=GBest.RA(i,Act(k));
            
        elseif H < -Alpha 
         %   disp('Lbest')
           Schedule(i,Act(k),6)=-1;
          
           TimeNew(i,Act(k),3)=PBest.RA(i,Act(k));
          ...
          % disp('Rand')
            ...
            TimeNew(i,Act(k),3)=TypeR(unidrnd(length(TypeR)));

        end      
                      
             RAsig=TimeNew(i,Act(k),3); % Asignaci??n del Recurso
                      
             TFS=Schedule(:,:,2);
                                       
             TFAmr=TFS(TimeNew(:,:,3)==RAsig); % Buscar si el recurso RAsig ha sido asignado anteriormente
             
             if isempty(TFAmr)==1 % Si el recurso asignado a la actividad j nunca ha sido asignado
                 
                 TimeNew(i,Act(k),1)=max(TimeNew(i,AA',2)); % Calcula tiempo de inicio de la actividad j del proyecto i

                 TimeNew(i,Act(k),2)=TimeNew(i,Act(k),1) + ModeProc(Act(k),RRHH(RAsig,3)); % Calcula tiempo de finalizaci??n de la actividad j del proyecto i
             
             else
                 
              TimeNew(i,Act(k),1)=max(max(TFAmr),max(TimeNew(i,AA',2))); % Calcula tiempo de inicio de la actividad j del proyecto i
             
               TimeNew(i,Act(k),2)=TimeNew(i,Act(k),1) + ModeProc(Act(k),RRHH(RAsig,3)); % Calcula tiempo de finalizaci??n de la actividad j del proyecto i
                             
             end
              
             Fnew=TimeNew(i,Act(k),1);
             
             Fold=PBest.TI(i,Act(k));
             ...
             if Dif <= 0 || Acept > rand
               ...
               Schedule(i,Act(k),1)=TimeNew(i,Act(k),1);
               
               Schedule(i,Act(k),2)=TimeNew(i,Act(k),2);
                
               Schedule(i,Act(k),3)=TimeNew(i,Act(k),3);
                
             else
                 
             RAsig=TimeOld(i,Act(k),3);%PBest.RA(i,Act(k));%
             
             Schedule(i,Act(k),3)=RAsig; % Asignaci??n del Recurso
